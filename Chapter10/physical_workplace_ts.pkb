...
begin
  v_id_context := get_id_context(ain_parent_id);
...
ain_id                         in     PHYSICAL_WORKPLACE_T.id%TYPE,
aid_inactive_date              in     PHYSICAL_WORKPLACE_T.inactive_date%TYPE) is
...
begin
  TEXT_HELP.process('PHYSICAL_WORKPLACE_TS');
end help;
...
begin
  ...
  if r_physical_workplace.id is not NULL then
    aior_physical_workplace.id := r_physical_workplace.id;
    aior_physical_workplace.parent_id             := r_physical_workplace.parent_id;
    aior_physical_workplace.id_context            := r_physical_workplace.id_context;
    if nvl(r_physical_workplace.workplace_type_id, n_null) <> nvl(aior_physical_workplace.workplace_type_id, n_null) or
       nvl(r_physical_workplace.code,              v_null) <> nvl(aior_physical_workplace.code,              v_null) or
       nvl(r_physical_workplace.name,              v_null) <> nvl(aior_physical_workplace.name,              v_null) or
       nvl(r_physical_workplace.active_date,       d_null) <> nvl(aior_physical_workplace.active_date,       d_null) or
       nvl(r_physical_workplace.inactive_date,     d_null) <> nvl(aior_physical_workplace.inactive_date,     d_null) then
      begin
        update PHYSICAL_WORKPLACE_T
        set    workplace_type_id     = aior_physical_workplace.workplace_type_id,
               code                  = aior_physical_workplace.code,
               name                  = aior_physical_workplace.name,
               active_date           = aior_physical_workplace.active_date,
               inactive_date         = aior_physical_workplace.inactive_date
        where  id = aior_physical_workplace.id;
...
--    r_physical_workplace.id := NULL;
  ...
  TEST_TS.set_test('PHYSICAL_WORKPLACE_TS', 'is_duplicate()', 11, 
  ...
  TEST_TS.set_test('PHYSICAL_WORKPLACE_TS', 'DELETE', 12, 
    'Delete test entries');
  begin
    delete PHYSICAL_WORKPLACE_T
    where  code in (
